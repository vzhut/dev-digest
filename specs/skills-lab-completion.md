...
**Calibration result, final (2026-09-22, `POST /pulls/:id/review`, every run checked for a real diff: `tokens_in` ≈ 2100+, `prompt_assembly.user` ≈ 4–6k chars).** `deepseek-v4-flash` with a role-level prompt and no skills flags *obvious* contract changes: PR #1 (rename/removal/route move) 7/8, PR #3 (nullable field, tightened validation, error-body rename) 6/6 — neither usable as the baseline. PR #4 (adds `cancelled` to `RunStatus`) went through two calibration rounds: the first (0/0/0) had a latent bug — `cancelRun` mutated the stored object, which the baseline correctly flagged as CRITICAL in 2 separate later runs, unrelated to the enum. Fixed (`cancelRun` returns a new object) and re-calibrated over **6** runs: **0/6 CRITICAL**, only unrelated WARNINGs (missing idempotence). With `api-contract-gate` **v1** (enum growth filed under Risky/WARNING): 3/4 CRITICAL, and separately 3/3 runs returned zero findings, reading the enum as "safe" — the skill's own wording put it two bullets above a "Safe: a new response field" line and the model conflated the two. Fixed in `api-contract-gate` v3 (server/src/db/seed.ts): enum growth is its own Breaking/CRITICAL paragraph with a worked bad/good switch-statement example. A second, unrelated bug then surfaced: two runs showed a red "rejected" badge with **zero findings** and score 100 — the model found the CRITICAL but cited the wrong line, grounding correctly dropped it, and `Review.verdict` was never recomputed post-grounding (fixed, `reviewer-core/INSIGHTS.md`). Final, re-verified over 6 runs: **6/6 CRITICAL**, all correctly grounded. Use **PR #4 at its current head** (two commits), skill **v3+**, and a server built from `c20a3e3`+ (verdict fix).
