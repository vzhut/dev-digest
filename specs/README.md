...
- [`devdigest-mcp.md`](devdigest-mcp.md) — L04 `devdigest-mcp`: local stdio MCP server (`mcp-server/`) over the REST API — five tools with final contracts and verbatim descriptions, state/error table, blocking run (D5), token budget, safety; task cards in [`devdigest-mcp.tasks.md`](devdigest-mcp.tasks.md); explained in [`../docs/devdigest-mcp.md`](../docs/devdigest-mcp.md).
