...
| mcp-server | `mcp-server/` | unit + in-memory MCP + stdio spawn (hermetic) | vitest | `mcp-server.yml` | no |
...
**mcp-server** — the local MCP server: tool handlers with a fake API client, `tools/list` contract and token budget over an in-memory MCP transport, and a stdio spawn against an unreachable API URL (clean stdout, fast `initialize`). No real API, no LLM.
